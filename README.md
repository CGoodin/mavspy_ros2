...
```bash
cd ~/mavs_ros2_ws/src
git clone https://github.com/CGoodin/nature-stack nature
...
```bash
ros2 launch mavspy_ros2 mavs_nature_sim.launch.py scene_file:=${MAVS_DATA}/scenes/cavs_proving_ground.json vehicle_file:=${MAVS_DATA}/vehicles/rp3d_vehicles/forester_2017_rp3d_tires.json waypoints_file:=$HOME/mavs_ros2_ws/src/nature/config/waypoints_proving_ground_enu.yaml robot_description_file:=$HOME/mavs_ros2_ws/src/nature/config/example_bot.urdf init_x:=-132.0 init_y:=318.0 init_heading:=-0.75 rain_rate:=0.0 lidar_model:=VLP-16
```
